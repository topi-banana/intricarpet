...
#[cfg(feature = "since-1.18")] import com.llamalad7.mixinextras.injector.WrapWithCondition;
#[cfg(feature = "since-1.21")] import java.util.List;
#[cfg(feature = "since-1.21.5")] import java.util.function.Consumer;
#[cfg(feature = "since-1.18")] import me.lntricate.intricarpet.interactions.Interaction;
#[cfg(feature = "since-1.18")] import me.lntricate.intricarpet.interfaces.IChunkMap;
#[cfg(feature = "since-1.18")] import net.minecraft.server.level.ChunkMap;
import net.minecraft.server.level.ServerChunkCache;
#[cfg(feature = "since-1.18")] import net.minecraft.server.level.ServerLevel;
#[cfg(feature = "since-1.18")] import net.minecraft.world.level.NaturalSpawner.SpawnState;
#[cfg(feature = "since-1.18")] import net.minecraft.world.level.chunk.LevelChunk;
#[cfg(feature = "since-1.18")] import org.spongepowered.asm.mixin.Final;
import org.spongepowered.asm.mixin.Mixin;
#[cfg(feature = "since-1.18")] import org.spongepowered.asm.mixin.Shadow;
#[cfg(feature = "since-1.18")] import org.spongepowered.asm.mixin.Unique;
#[cfg(feature = "since-1.18")] import org.spongepowered.asm.mixin.injection.At;
#[cfg(feature = "since-1.21.5")] import org.spongepowered.asm.mixin.injection.Redirect;
    ...
    @Redirect(
        ...
        at =
            @At(
                value = "INVOKE",
                target =
                    "Lnet/minecraft/server/level/ChunkMap;forEachBlockTickingChunk(Ljava/util/function/Consumer;)V"))
    private void redirectForEachBlockTickingChunk(
        ChunkMap chunkMapInstance, Consumer<LevelChunk> originalConsumer) {
        Consumer<LevelChunk> wrapper =
            (levelChunk) -> {
                try {
                    boolean should =
                        ((IChunkMap) chunkMapInstance)
                            .anyPlayerCloseWithInteraction(
                                levelChunk.getPos(), Interaction.RANDOMTICKS);
                    if (should) {
